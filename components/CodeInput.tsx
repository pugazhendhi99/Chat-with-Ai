...
export default function CodeInput({ onSend, loading }: { onSend: (data: { code: string; question: string }) => void; loading: boolean }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 bg-gradient-to-br from-white/90 via-blue-50 to-pink-50 p-5 rounded-2xl shadow-xl flex flex-col gap-4 border border-gray-100 transition-all duration-700">
      <input
        type="text"
        value={question}
      ...
      <div className="flex gap-2 items-center">
        ...
        />
        {fileName && (
          <span className="text-xs text-gray-700 bg-gradient-to-r from-gray-100 to-pink-50 px-2 py-1 rounded flex items-center gap-1 shadow-sm">
            {fileName}
      ...
      <textarea
        ...
        className="w-full h-32 p-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-200 font-mono text-sm shadow-sm transition-all duration-300"
        disabled={loading || !!fileName}
      />
      <button
        type="submit"
        className="self-end bg-gradient-to-r from-blue-500 to-pink-500 hover:from-blue-600 hover:to-pink-600 text-white font-semibold px-6 py-2 rounded-lg shadow-md transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
